from DQN_Brain import DQN, DataWithTest
import sys
import os
import cv2
import numpy as np
...
def train(it, avg, high):
    for eps in range(train_step):
        ...
        print(f"model: {prefix} training: iter: [{it}] eps: [{eps}] avg: [{avg}] high: [{high}]")
...
def run_maze():
    ...
    while True:
        ...
        # 写入文件
        with open("../dqn.txt", 'a', encoding='utf-8') as f:
            f.write(f"{avg} {high}\n")
...
if __name__ == "__main__":
    ...
    try:
        model.load_weights(f"../models/test.h5")
        print("模型权重加载成功")
    except:
        print("模型权重加载失败")
